    ...
    <div ref={ref} className="relative ml-auto shrink-0">
      ...
      {open && (
        <div className="absolute right-0 top-7 z-50 w-36 rounded-lg border border-slate-200 bg-white shadow-lg py-1">
          ...
          >
            <span className="material-icons text-slate-400" style={{ fontSize: '0.85rem' }}>
              restore
            </span>
            Restore
          </button>
          ...
          >
            <span className="material-icons text-red-400" style={{ fontSize: '0.85rem' }}>
              delete
            </span>
            Delete
          </button>
    ...
    <div className="max-h-72 overflow-y-auto rounded-lg border border-dashed border-slate-300/80 bg-slate-50 p-3">
      {versions.length === 0 ? (
        <p className="text-sm text-slate-500">
          No saved content yet. Start typing to create snapshots.
        </p>
      ) : (
        <ul className="space-y-2">
          {versions.map((version) => (
            <li
              key={version.id}
              className="flex items-start gap-1 rounded-md border border-slate-200 bg-white px-3 py-2"
            >
              <div className="min-w-0 flex-1">
                <p className="line-clamp-2 text-xs text-slate-700">
                  {version.preview || 'Untitled content'}
                </p>
